...
namespace cones
{
    ...
    struct FuncDef
    { // Struct containing arguments, a description, the number of arguments to expect, and the evaluation and unit transformation functions
        std::string args;
        std::string desc;
        int num_args;
    ...
    inline const std::map<std::string, FuncDef> &get_builtin_math_functions()
    {
        static const std::map<std::string, FuncDef> funcs = {
            // TRIGONOMETRY
            {"sin", {"x", "Trigonometric sine (radians)", 1, [](const std::vector<DualNumber> &a)
                     { return DualNumber(std::sin(a[0].val), std::cos(a[0].val) * a[0].der); }, [](const std::vector<Unit> &)
                     { return Unit::Dimensionless(); }}},
            {"cos", {"x", "Trigonometric cosine (radians)", 1, [](const std::vector<DualNumber> &a)
                     { return DualNumber(std::cos(a[0].val), -std::sin(a[0].val) * a[0].der); }, [](const std::vector<Unit> &)
                     { return Unit::Dimensionless(); }}},
            {"tan", {"x", "Trigonometric tangent (radians)", 1, [](const std::vector<DualNumber> &a)
                     { return DualNumber(std::tan(a[0].val), (1.0 / std::pow(std::cos(a[0].val), 2)) * a[0].der); }, [](const std::vector<Unit> &)
                     { return Unit::Dimensionless(); }}},
            // INVERSE TRIG
            {"asin", {"x", "Inverse sine", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::asin(a[0].val), a[0].der / std::sqrt(1.0 - a[0].val * a[0].val)); }, [](const std::vector<Unit> &)
                      { return Unit::Dimensionless(); }}},
            {"acos", {"x", "Inverse cosine", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::acos(a[0].val), -a[0].der / std::sqrt(1.0 - a[0].val * a[0].val)); }, [](const std::vector<Unit> &)
                      { return Unit::Dimensionless(); }}},
            {"atan", {"x", "Inverse tangent", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::atan(a[0].val), a[0].der / (1.0 + a[0].val * a[0].val)); }, [](const std::vector<Unit> &)
                      { return Unit::Dimensionless(); }}},
            // HYPERBOLIC TRIG
            {"sinh", {"x", "Hyperbolic sine", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::sinh(a[0].val), std::cosh(a[0].val) * a[0].der); }, [](const std::vector<Unit> &)
                      { return Unit::Dimensionless(); }}},
            {"cosh", {"x", "Hyperbolic cosine", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::cosh(a[0].val), std::sinh(a[0].val) * a[0].der); }, [](const std::vector<Unit> &)
                      { return Unit::Dimensionless(); }}},
            {"tanh", {"x", "Hyperbolic tangent", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::tanh(a[0].val), (1.0 / std::pow(std::cosh(a[0].val), 2)) * a[0].der); }, [](const std::vector<Unit> &)
                      { return Unit::Dimensionless(); }}},

            // COMMON ROOTS/POWERS
            {"sqrt", {"x", "First (positive) square root", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::sqrt(a[0].val), (0.5 / std::sqrt(a[0].val)) * a[0].der); }, [](const std::vector<Unit> &u)
                      { return u[0].pow(0.5); }}},
            {"cbrt", {"x", "First cube root", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::cbrt(a[0].val), (1 / (3 * std::sqrt(a[0].val))) * a[0].der); }, [](const std::vector<Unit> &u)
                      { return u[0].pow(0.5); }}},
            {"pow", {"x, d", "Raises x to the dth power", 2, [](const std::vector<DualNumber> &a) // TODO: Change from current assumed-static approach for the degree
                     { return DualNumber(std::pow(a[0].val, a[1].val), (a[1].val / (std::pow(a[0].val, a[1].val))) * a[0].der); }, [](const std::vector<Unit> &u)
                     { return u[0].pow(0.5); }}},

            // LOGARITHMIC AND EXPONENTIAL FUNCTIONS
            {"log", {"x", "Natural logarithm (base-e)", 1, [](const std::vector<DualNumber> &a)
                     { return DualNumber(std::log(a[0].val), a[0].der / a[0].val); }, [](const std::vector<Unit> &)
                     { return Unit::Dimensionless(); }}},
            {"log10", {"x", "Common logarithm (base-10)", 1, [](const std::vector<DualNumber> &a)
                       { return DualNumber(std::log10(a[0].val), a[0].der / (a[0].val * std::log(10.0))); }, [](const std::vector<Unit> &)
                       { return Unit::Dimensionless(); }}},
            {"exp", {"x", "Exponential function (e^x)", 1, [](const std::vector<DualNumber> &a)
                     { return DualNumber(std::exp(a[0].val), std::exp(a[0].val) * a[0].der); }, [](const std::vector<Unit> &)
                     { return Unit::Dimensionless(); }}},

            // MATH CONVENIENCE FUNCTIONS
            {"abs", {"x", "Absolute value", 1, [](const std::vector<DualNumber> &a)
                     { return DualNumber(std::abs(a[0].val), (a[0].val >= 0 ? 1.0 : -1.0) * a[0].der); }, [](const std::vector<Unit> &u)
                     { return u[0]; }}},
            {"round", {"x", "Round to nearest integer", 1, [](const std::vector<DualNumber> &a)
                       { return DualNumber(std::round(a[0].val), 0.0); }, [](const std::vector<Unit> &u)
                       { return u[0]; }}},
            {"floor", {"x", "Floor function", 1, [](const std::vector<DualNumber> &a)
                       { return DualNumber(std::floor(a[0].val), 0.0); }, [](const std::vector<Unit> &u)
                       { return u[0]; }}},
            {"ceil", {"x", "Ceiling function", 1, [](const std::vector<DualNumber> &a)
                      { return DualNumber(std::ceil(a[0].val), 0.0); }, [](const std::vector<Unit> &u)
                      { return u[0]; }}}};
        return funcs;
    }
    ...
    {
        static const std::map<std::string, FuncDef> funcs = {
            {"Q_conv", {"h, A, Ts, Tinf", "Newton's Law of Cooling: Q = h*A*(Ts - Tinf)", 4, [](const std::vector<DualNumber> &a)
                        { return a[0] * a[1] * (a[2] - a[3]); }, [](const std::vector<Unit> &u)
                        { return u[0] * u[1] * u[2]; }}},
            {"Q_cond", {"k, A, dT, L", "Fourier's Law (uniform cross-sectional area): Q = k*A*dT/L", 4, [](const std::vector<DualNumber> &a)
                        { return (a[0] * a[1] * a[2]) / a[3]; }, [](const std::vector<Unit> &u)
                        { return (u[0] * u[1] * u[2]) / u[3]; }}},
            {"Q_rad", {"eps, A, Ts, Tsur", "Surface-Surroundings radiation. Stefan-Boltzmann Law: Q = eps*sigma*A*(Ts^4 - Tsur^4)", 4, [](const std::vector<DualNumber> &a)
                       { 
                const double sigma = 5.670374419e-8; // TODO: Change to use the actual constants
                return a[0] * sigma * a[1] * (a[2].pow(4) - a[3].pow(4)); }, [](const std::vector<Unit> &)
                       {
    ...
    public:
        ...
        DualNumber evaluate(const std::vector<FuncArg> &args) const override
        {
            ...
            for (const auto &arg : args)
            {
                ...
                // Explicit naming or positional variable name
                if (!arg.name.empty())
                {
                ...
                // Unit Inference (if pt is still unknown)
            ...
            if (!substance)
                throw std::runtime_error("Substance not found: " + material);
        ...
        Unit get_unit(const std::vector<Unit> &) const override
        {
            ...
            case PropertyType::ENTHALPY:
                return Unit(1.0, {0, 2, -2, 0, 0}); // J/kg
            case PropertyType::ENTROPY:
                return Unit(1.0, {0, 2, -2, -1, 0}); // J/kg*K
            case PropertyType::INTERNAL_ENERGY:
                return Unit(1.0, {0, 2, -2, 0, 0}); // J/kg
            case PropertyType::SPECIFIC_VOLUME:
                return Unit(1.0, {-1, 3, 0, 0, 0}); // m^3/kg
            case PropertyType::DENSITY:
                return Unit(1.0, {1, -3, 0, 0, 0}); // kg/m^3
            case PropertyType::QUALITY:
                return Unit::Dimensionless();
            case PropertyType::VISCOSITY:
                return Unit(1.0, {1, -1, -1, 0, 0}); // Pa*s = kg/m*s
            case PropertyType::CONDUCTIVITY:
                return Unit(1.0, {1, 1, -3, -1, 0}); // W/m*K = kg*m/s^3*K
