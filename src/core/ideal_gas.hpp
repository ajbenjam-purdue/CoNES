...
namespace cones
{
    ...
    public:
        ...
        // Evaluate the idealGas at the property inputs
        DualNumber evaluate(PropertyType target, const std::vector<PropertyArg> &inputs) const override
        {
            ...
            for (const auto &in : inputs)
            {
                ...
                if (in.type == PropertyType::DENSITY)
                {
                    rho = in.value;
                    has_rho = true;
                }
                ...
                if (in.type == PropertyType::INTERNAL_ENERGY)
                {
                    u = in.value;
                    has_u = true;
                }
                if (in.type == PropertyType::SPECIFIC_VOLUME)
                {
                    v = in.value;
                    has_v = true;
                }
            ...
            if (target == PropertyType::PRESSURE && has_P)
                return P;
            if (target == PropertyType::TEMPERATURE && has_T)
                return T;
            if (target == PropertyType::ENTHALPY && has_T)
                return T * Cp_;
            if (target == PropertyType::ENTROPY && has_T && has_P)
                return Cp_ * log(T / T_ref) - R_gas_ * log(P / P_ref);
            if (target == PropertyType::INTERNAL_ENERGY && has_T)
                return T * (Cp_ - R_gas_);
        ...
        // Clone of above but for DualRow
        DualRow evaluate(PropertyType target, const std::vector<PropertyArgRow> &inputs) const override
        {
            ...
            DualRow T(0, der_size), P(0, der_size), rho(0, der_size), h(0, der_size), u(0, der_size), v(0, der_size), s(0, der_size);
            bool has_T = false, has_P = false, has_rho = false, has_h = false, has_u = false, has_v = false, has_s = false;
